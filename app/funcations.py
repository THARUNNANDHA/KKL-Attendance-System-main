    ...
    if os.path.exists(file_path):
        ...
        for sheet_name in sheet_names:
            ...
            for index, row in df.iterrows():
                ...
                today_date = datetime.now().strftime("%d-%m-%Y")  # Use the format that matches your database
                print(today_date)

                matching_holidays = Festival.query.filter(func.DATE(Festival.date) == today_date).all()
                # Assuming Week_off is defined somewhere
                week_off = Week_off.query.all()

                if matching_holidays:
                    for holiday in matching_holidays:
                        print(f'Today is a holiday: {holiday.holiday}')
